  ...
  <Teleport to="body">
    <Transition name="modal-fade">
      <div v-if="open" class="modal-overlay" @click.self="$emit('close')">
        <div ref="modalEl" class="modal" role="dialog" aria-modal="true" aria-label="Sign in" @keydown.escape="$emit('close')" @keydown="trapFocus">
          ...
          <div class="oauth-section">
            <button class="oauth-btn" @click="handleOAuth('github')" :disabled="busy">
              <span v-if="busyProvider === 'github'" class="spinner spinner-oauth" aria-hidden="true" />
            ...
            <button class="oauth-btn" @click="handleOAuth('google')" :disabled="busy">
              <span v-if="busyProvider === 'google'" class="spinner spinner-oauth" aria-hidden="true" />
            ...
            <button class="oauth-btn" @click="handleOAuth('facebook')" :disabled="busy">
              <span v-if="busyProvider === 'facebook'" class="spinner spinner-oauth" aria-hidden="true" />
              <svg v-else width="18" height="18" viewBox="0 0 24 24" fill="#1877F2" aria-hidden="true">
          ...
          <div class="tabs">
            <button
              ...
              @click="tab = 'login'; errorMsg = ''; successMsg = ''"
            >Log in</button>
            <button
              class="tab-btn"
              :class="{ active: tab === 'signup' }"
              @click="tab = 'signup'; errorMsg = ''; successMsg = ''"
            >Sign up</button>
          </div>
...
// A2 + A3: Focus first element on open
...
function trapFocus(e) {
  ...
  const el = modalEl.value
  if (!el) return
  const focusable = el.querySelectorAll('button:not([disabled]), input:not([disabled]), [tabindex]:not([tabindex="-1"])')
